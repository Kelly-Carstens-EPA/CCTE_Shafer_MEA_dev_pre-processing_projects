...
spidmap3 <- as.data.table(read.xlsx(file.path(root_output_dir,'Sample IDs/EPA_ES209_EPA-Shafer_2_20210427_key.xlsx'), sheet = 1))
spidmap3[, treatment := PREFERRED_NAME]
spidmap3[, expected_stock_conc := TARGET_CONCENTRATION]
unique(spidmap3$TARGET_CONCENTRATION_UNIT) # mM
...
# 4/27/2021 -> adding filler spids for the tested compounds that dont' have a spid at the moment
add.spid.tb <- data.table(treatment = c('Bisphenol A','Acetaminophen'), spid = c('Bisphenol A','Acetaminophen'), expected_stock_conc = c(30,30), stock_conc = c(30,30))
spidmap <- rbind(spidmap, add.spid.tb, fill = T)

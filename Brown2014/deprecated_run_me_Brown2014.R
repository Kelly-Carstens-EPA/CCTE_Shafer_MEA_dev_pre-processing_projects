...
# we are not including the data for Glyphosate
...
setnames(spidmap, old = c("ALIQUOT_CONCENTRATION","EPA_SAMPLE_ID"), new = c("stock_conc","spid"))
...
spidmap[, treatment := PREFERRED_NAME]
...
setnames(spidmap2, old = c("ALIQUOT_CONCENTRATION","EPA_SAMPLE_ID","TARGET_CONCENTRATION"), new = c("stock_conc","spid","expected_stock_conc"))
# for example, setnames(spidmap, old = c("Aliquot_Vial_Barcode", "Concentration", "EPA_Sample_ID"), new = c("treatment","stock_conc","spid"))
spidmap2[, treatment := PREFERRED_NAME]
spidmap2[, treatment := as.character(treatment)]
spidmap2[, stock_conc := as.numeric(stock_conc)]
usecols <- c("PREFERRED_NAME","spid","treatment","stock_conc","expected_stock_conc")
...
spidmap[PREFERRED_NAME == "Bisindolylmaleimide I", treatment := "Bisindolymaleimide 1"] # Shafer_42 list CASRN is 133052-90-1, which matches the casn shown in paper
spidmap[PREFERRED_NAME == "L-Domoic acid", treatment := "Domoic Acid"] # Shafer_42 CASRN is 14277-97-5, which matches the casn shown in the paper
spidmap[PREFERRED_NAME == "Sodium orthovanadate", treatment := "Sodium Orthovanadate"] # just change in captilization
spidmap[PREFERRED_NAME == "4-(4-Chlorophenyl)-4-hydroxy-N,N-dimethyl-alpha,alpha-diphenylpiperidine-1-butyramide monohydrochloride", treatment := "Loperamide"]
spidmap <- spidmap[!(PREFERRED_NAME == "Loperamide" & spid == 'EX000492')] # removing this spid for Loperamide (not Loperamide hydrochloride)

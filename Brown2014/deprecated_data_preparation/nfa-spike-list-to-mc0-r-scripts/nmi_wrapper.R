...
nmi_wrapper<-function(mutdata){
  ...
  for (i in 1:ncol(testing)) {
    ...
    # for some h5 files, some of the rows from the meta data are duplicated in the h5 file...
    if (nrow(meta[[i]]) != 48) {
      warning(paste0("meta[[",i,"]] in spikeLoadRountines has ",nrow(meta[[i]])," rows"))
      meta[[i]] <- unique(meta[[i]])
    }
    if (length(testing[,i]) != 48) {
      stop(paste0("testing[[,",i,"]] in spikeLoadRountines has ",nrow(testing[,i])," rows"))
    }

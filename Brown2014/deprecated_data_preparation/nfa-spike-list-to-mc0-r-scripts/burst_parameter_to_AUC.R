...
filename = "Brown2016_AUC.csv"
# set file with the 16 parameters for all culture dates
parameter_data = read.csv("L:/Lab/NHEERL_MEA/Carpenter_Amy/pre-process_old_data_for_tcpl/Brown2016/Intermediate_output/Brown2016_all_prepared_data_added_DIV9_median.csv", sep = ",")
# set file with mi data for all culture dates
mi_data = read.csv("L:/Lab/NHEERL_MEA/Carpenter_Amy/pre-process_old_data_for_tcpl/Brown2016/Intermediate_output/Brown2016_MI_added_DIV9_median.csv", sep = ",")

##########################################################
# END USER INPUTS
##########################################################

# -------------- addition for Brown 2016 data set
# found that some h5 files were named differently, so "DIV" word included in that column
# will remove now
parameter_data[, "DIV"] <- as.numeric(sub(pattern="(DIV)0*","",parameter_data$DIV))
mi_data[, "DIV"] <- as.numeric(sub(pattern="(DIV)0*","",mi_data$DIV))
# ----------------------------------

...
library(data.table)
approximate_DIV_by_median <- function(dat, platei, cplates, add.DIV)
{
  ...
  dat[dose == 0, trt := "Control"]
  plate.dat.template[dose == 0, trt:= "Control"]
  ...
  add.dat <- dat[Plate.SN %in% cplates & DIV == add.DIV, lapply(usecols, function(x) median(get(x), na.rm = T)), by = c("trt","dose","units")]
  setnames(add.dat, old = paste0("V",1:length(usecols)), new = usecols)
  add.dat <- merge(plate.dat.template, add.dat, by = c("trt","dose","units"))
  add.dat[, `:=`(DIV = add.DIV, file.name = paste0("median_of_DIV9_in_corresponding_wells_of_",cplates[1],",",cplates[2]))]
  return(add.dat)
}
...
outfile <- "L:/Lab/NHEERL_MEA/Carpenter_Amy/pre-process_old_data_for_tcpl/Brown2016/Intermediate_output/Brown2016_all_prepared_data_added_DIV9_median.csv"
...
div9.dat <- approximate_DIV_by_median(pdat, platei = platei, cplates = cplates, add.DIV = 9)
...
outfile2 <- "L:/Lab/NHEERL_MEA/Carpenter_Amy/pre-process_old_data_for_tcpl/Brown2016/Intermediate_output/Brown2016_MI_added_DIV9_median.csv"

# run the function, append the data, and save it
rm(div9.dat)
div9.dat <- approximate_DIV_by_median(midat, platei = "MW1230-53", cplates = cplates, add.DIV = 9)

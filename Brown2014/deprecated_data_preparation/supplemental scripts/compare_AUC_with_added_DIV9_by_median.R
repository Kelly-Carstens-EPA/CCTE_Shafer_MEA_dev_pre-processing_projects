...
graphics.off()
pdf(file = file.path(paste0('figs/',platei,'_comparison_control_wells_with_updates_bymedian.pdf')), height = 6, width = 10, pointsize = 5)
...
graphics.off()
pdf(file = file.path(paste0('figs/',platei,'_comparison_all_wells_with_updates_bymedian.pdf')), height = 6, width = 10, pointsize = 5)
par(mfrow = c(3,3), oma = c(0,0,6,0), mar = c(4, 4, 1.5, 2), xpd = TRUE)
for (endpoint in endpoint_cols) {
  setnames(x = auc1, old = endpoint, new = "current_endpoint")
...
graphics.off()
pdf(file = file.path(paste0('figs/',platei,'_comparison_4_endpoints_by_div_with_updates_bymedian.pdf')), height = 6, width = 10, pointsize = 6)
par(mfrow = c(2,2))

...
# Prepare "mea_treatment_name" col in spidmap to match MEA treatment names ----------------
spidmap[, notes := ""]
...
spidmap[primary_dataset == "Frank2017" & casn %in% unique(table1$casn), c("mea_treatment_name") := .(table1[match(spidmap[primary_dataset == "Frank2017" & casn %in% unique(table1$casn),casn], table1$casn), c(treatment)])]
...
spidmap[!is.na(Compound.name.in.MEA.data), mea_treatment_name := Compound.name.in.MEA.data]
...
# -------------------------
# spidmap[primary_dataset %in% c("Frank2017","DNTGF2019"), mea_treatment_name := treatment]
# spidmap[primary_dataset %in% c("Frank2017","DNTGF2019"), treatment := PREFERRED_NAME]
# name_map1 <- as.data.table(read.csv(file.path(root_output_dir, "supplemental_mea_treatment_name_map.csv"), stringsAsFactors = F))
# spidmap_test <- merge(spidmap, name_map1, by.x = c("treatment"), by.y = c("spid_treatment_name"), all = T, suffixes = c("",".add"))
# # not sure why there are 4 added rows now (516 to 523)
# spidmap_test[is.na(mea_treatment_name), mea_treatment_name := treatment]

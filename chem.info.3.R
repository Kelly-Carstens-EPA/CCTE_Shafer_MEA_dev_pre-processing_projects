chem.info.3<-function (file, masterChemFile = masterChemFile) 
{
  ...
  plate.chem.info$well <- as.character(masterCD$Well)
  plate.chem.info$treatment <- as.character(masterCD$Treatment)
  plate.chem.info$dose <- as.character(masterCD$Dose)
  plate.chem.info$size <- as.character(masterCD$Size)
  plate.chem.info$units <- as.character(masterCD$Units)
  
  # added in these values March 26, 2020
  plate.chem.info$Experiment.Date <- as.character(masterCD$Experiment.Date)
  plate.chem.info$Plate.SN <- as.character(masterCD$Plate.SN)
  
  # the previous method to add data to plate.chem.info:

  # not sure when needing to check shortFileName represnted in masterCD would be important
  # count = 1
